...
$do$
BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ValidateGstr1aSummaryType' AND typnamespace = 'oregular'::regnamespace::oid)
THEN
    CREATE TYPE "oregular"."ValidateGstr1aSummaryType" AS ("EntityId" integer, "SummaryType" smallint, "DifferentialPercentage" numeric, "Gstin" character varying, "Pos" smallint, "ReturnPeriod" integer, "FinancialYear" integer, "Rate" numeric, "Hsn" character varying, "Uqc" character varying, "Description" character varying, "IsAmendmment" boolean, "OriginalPeriod" integer, "NilExemptNonGstType" smallint, "DocumentType" smallint, "GstActOrRuleSectionType" smallint, "DocumentSeriesFrom" character varying, "DocumentSeriesTo" character varying, "GroupId" integer, "HasError" boolean);
